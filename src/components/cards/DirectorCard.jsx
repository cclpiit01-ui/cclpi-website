...
const DirectorCard = ({ name, role, company, image, delay }) => {
  return (
    <Reveal direction="bottom" delay={delay}>
      <div className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 border-b-[10px] border-b-brand-primary border-brand-primary hover:shadow-2xl hover:shadow-brand-primary/10 hover:-translate-y-2 transition-all duration-500 flex flex-col md:flex-row items-center md:items-stretch h-full">
        ...
        {/* Image Container */}
        <div className="w-full md:w-48 lg:w-56 bg-brand-surface overflow-hidden shrink-0 relative">
          <img 
            src={image} 
            alt={name} 
        ...
        <div className="p-8 flex flex-col justify-center flex-grow text-center md:text-left">
          <div className="mb-2">
            <span className="text-brand-secondary font-sans font-black italic text-[10px] uppercase tracking-[0.2em] bg-brand-secondary/10 px-3 py-1 rounded-full">
              {role}
            </span>
          </div>
          
          <h3 className="text-brand-primary font-heading font-black text-2xl uppercase leading-tight mb-3 tracking-tight">
            {name}
          </h3>
